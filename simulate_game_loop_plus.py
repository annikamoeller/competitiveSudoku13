...
def simulate_game(initial_board: SudokuBoard,
                  player1: SudokuAI,
                  player2: SudokuAI,
...
def play_game(board_file: Optional[str], name1: str, name2: str, calculation_time: float, verbose=True) -> GameResult:
    """
    ...
    p1_wins = 0
    p2_wins = 0
    board_name = board_file.replace("boards/", '')
    save_file = "experiment_results_with_score.txt"
    key_name = f"{name2}_{board_name}_{calculation_time}.txt"
    
    for i in range(5):
        print("loop ", i)
        player1_wins, player2_wins = simulate_game(board, player1, player2, calculation_time=calculation_time, verbose=verbose)
        p1_wins += player1_wins
        p2_wins += player2_wins
    
    p1_win_percentage = (p1_wins/5)*100
    result = {key_name: p1_win_percentage}
    f = open(save_file, "a")
    write_string = f"{result}\n"
    f.write(write_string)
    f.close()

    return simulate_game(board, player1, player2, calculation_time=calculation_time, verbose=verbose)
...
def main():
    ...
    if args.check:
        ...
        play_game(args.board, args.first, args.second, args.time, verbose = not args.quiet)
